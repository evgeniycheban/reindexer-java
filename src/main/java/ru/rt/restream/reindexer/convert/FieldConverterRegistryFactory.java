...
public enum FieldConverterRegistryFactory implements FieldConverterRegistry {
    ...
    private final Map<Class<?>, FieldConverter<?, ?>> globalConverters = new HashMap<>();
     ...
     * If the {@link FieldConverter} is not mapped directly for the field, looks up for a global one registered via
     * {@link #registerGlobalConverter(Class, FieldConverter)}, if global {@code FieldConverter} is found checks
    ...
    public <X, Y> FieldConverter<X, Y> getFieldConverter(Field field) {
        ...
        try {
            ...
            converter = globalConverters.get(field.getType());
        } finally {
        ...
        if (convert != null && convert.converterClass() != FieldConverter.class
            ...
            try {
                ...
                if (converter == null || converter.getClass() != convert.converterClass()) {
                    converter = converters.computeIfAbsent(convert.converterClass(), this::instantiateAttributeConverter);
     ...
     */
    @Override
    public void registerGlobalConverter(Class<?> fieldType, FieldConverter<?, ?> converter) {
        Objects.requireNonNull(fieldType, "fieldType must not be null");
        Objects.requireNonNull(converter, "converter must not be null");
        lock.writeLock().lock();
        try {
            FieldConverter<?, ?> prev = globalConverters.put(fieldType, converter);
            if (LOGGER.isTraceEnabled()) {
                if (prev == null) {
                    LOGGER.trace("Registered global converter {} for {}", converter, fieldType.getName());
                } else {
                    LOGGER.trace("Global converter: {} was overridden to {} for {}", prev, converter, fieldType.getName());
                }
            }
        } finally {
            lock.writeLock().unlock();
    ...
    private FieldConverter<?, ?> instantiateAttributeConverter(Class<?> converterClass) {
        try {
