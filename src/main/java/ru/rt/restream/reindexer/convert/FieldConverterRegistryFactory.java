...
public enum FieldConverterRegistryFactory implements FieldConverterRegistry {
    ...
    private final Map<Class<?>, GenericFieldConverter<?, ?>> converters = new HashMap<>();

    private final Map<ResolvableType, GenericFieldConverter<?, ?>> globalConverters = new HashMap<>();

    private final Map<Pair<Class<?>, String>, GenericFieldConverter<?, ?>> fieldConverters = new HashMap<>();

    /**
     * Returns a {@link GenericFieldConverter} that is mapped for the given {@code field} or {@literal null}.
     * The {@code X} and {@code Y} parameters are resolved and accessible via {@code GenericFieldConverter#getConvertiblePair}. 
     ...
     * @return the {@link GenericFieldConverter} to use
     */
    @SuppressWarnings("unchecked")
    public <X, Y> GenericFieldConverter<X, Y> getFieldConverter(Field field) {
        ...
        GenericFieldConverter<?, ?> converter;
        lock.readLock().lock();
        try {
        ...
        if (converter != null) {
             ...
             */
            return (GenericFieldConverter<X, Y>) converter;
        }
        ...
        if (convert != null && convert.converterClass() != FieldConverter.class
            && (converter == null || converter.getConverterClass() != convert.converterClass())) {
            lock.writeLock().lock();
            try {
                converter = fieldConverters.get(key);
                if (converter != null) {
                ...
                converter = globalConverters.get(fieldType);
                if (converter == null || converter.getConverterClass() != convert.converterClass()) {
                    converter = converters.computeIfAbsent(convert.converterClass(), k -> {
                        Pair<ResolvableType, ResolvableType> convertiblePair = ConversionUtils.resolveConverterTypePair(k);
                        FieldConverter<?, ?> fieldConverter = instantiateFieldConverter(k);
                        return new GenericFieldConverter<>(fieldConverter, convertiblePair);
                    });
    ...
    public void registerFieldConverter(Class<?> itemClass, String fieldName, FieldConverter<?, ?> converter) {
        ...
        GenericFieldConverter<?, ?> genericFieldConverter = getGenericFieldConverter(converter);
        lock.writeLock().lock();
        try {
            FieldConverter<?, ?> prev = fieldConverters.put(new Pair<>(itemClass, fieldName), genericFieldConverter);
    ...
    public void registerGlobalConverter(FieldConverter<?, ?> converter) {
        ...
        GenericFieldConverter<?, ?> genericFieldConverter = getGenericFieldConverter(converter);
        Pair<ResolvableType, ResolvableType> convertiblePair = genericFieldConverter.getConvertiblePair();
        lock.writeLock().lock();
        try {
            FieldConverter<?, ?> prev = globalConverters.put(convertiblePair.getFirst(), genericFieldConverter);
            if (LOGGER.isTraceEnabled()) {
                if (prev == null) {
    ...
    private GenericFieldConverter<?, ?> getGenericFieldConverter(FieldConverter<?, ?> converter) {
        if (converter instanceof GenericFieldConverter<?, ?>) {
            return (GenericFieldConverter<?, ?>) converter;
        }
        Pair<ResolvableType, ResolvableType> convertiblePair = ConversionUtils.resolveConverterTypePair(converter.getClass());
        return new GenericFieldConverter<>(converter, convertiblePair);
    }
