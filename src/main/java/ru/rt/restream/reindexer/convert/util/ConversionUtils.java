...
public final class ConversionUtils {
    ...
    /**
     * Resolves source and target types based on {@code X} and {@code Y} parameters from {@link FieldConverter} interface implementation.
     * If {@code FieldConverter} source or target type is an array, the type is determined by {@code Class#getComponentType},
     ...
     */
    public static Pair<ResolvableType, ResolvableType> resolveConverterTypePair(Class<?> converterClass) {
        Map<TypeVariable<?>, Type> typeArguments = TypeUtils.getTypeArguments(converterClass, FieldConverter.class);
        Type sourceType = typeArguments.get(FieldConverter.class.getTypeParameters()[0]);
        ResolvableType resolvableSourceType = resolveType(sourceType);
        if (resolvableSourceType == null) {
            throw new IllegalArgumentException(
                    String.format("Cannot resolve FieldConverter: %s source type: %s", converterClass.getName(), sourceType));
        }
        Type targetType = typeArguments.get(FieldConverter.class.getTypeParameters()[1]);
        ResolvableType resolvableTargetType = resolveType(targetType);
        if (resolvableTargetType == null) {
            throw new IllegalArgumentException(
                    String.format("Cannot resolve FieldConverter: %s target type: %s", converterClass.getName(), targetType));
        }
